...
export function MobileNav({ onCreate, onFilters, onProfile }: Props) {
  return (
    <nav className="fixed inset-x-3 bottom-3 z-40 flex items-center justify-around rounded-2xl border border-slate-200 bg-white/92 px-3 py-2 shadow-glass backdrop-blur-xl dark:border-white/10 dark:bg-slate-950/92 lg:hidden">
      <button className="grid h-11 w-11 place-items-center rounded-xl text-slate-700 dark:text-slate-200" aria-label="Canvas">
        <Home size={21} />
      </button>
      <button className="grid h-11 w-11 place-items-center rounded-xl text-slate-700 dark:text-slate-200" aria-label="Explore">
        <Compass size={21} />
      </button>
