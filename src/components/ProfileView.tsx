      ...
      <form onSubmit={submit} onPointerDown={(event) => event.stopPropagation()} className="modal-scroll-pane thin-scrollbar flex max-h-[92dvh] w-full max-w-2xl flex-col overflow-hidden rounded-t-[28px] border border-slate-200 bg-white shadow-2xl dark:border-white/10 dark:bg-[#0f1115] sm:rounded-[28px]">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <label>
            ...
            <input value={form.displayName} onChange={(event) => update("displayName", event.target.value)} className="w-full rounded-2xl border border-slate-200 bg-transparent px-4 py-3 outline-none focus:border-[#A0845C] dark:border-white/10" />
          </label>
          <label>
            <span className="mb-2 block text-sm font-semibold">Username</span>
            <input value={form.username} onChange={(event) => update("username", event.target.value)} className="w-full rounded-2xl border border-slate-200 bg-transparent px-4 py-3 outline-none focus:border-[#A0845C] dark:border-white/10" />
          </label>
          <label>
            <span className="mb-2 block text-sm font-semibold">Location</span>
            <input value={form.location} onChange={(event) => update("location", event.target.value)} className="w-full rounded-2xl border border-slate-200 bg-transparent px-4 py-3 outline-none focus:border-[#A0845C] dark:border-white/10" />
          </label>
          <label>
            <span className="mb-2 block text-sm font-semibold">Website</span>
            <input value={form.website} onChange={(event) => update("website", event.target.value)} className="w-full rounded-2xl border border-slate-200 bg-transparent px-4 py-3 outline-none focus:border-[#A0845C] dark:border-white/10" />
          </label>
        </div>
        <label className="mt-4 block">
          <span className="mb-2 block text-sm font-semibold">Bio</span>
          <textarea value={form.bio} onChange={(event) => update("bio", event.target.value)} className="min-h-28 w-full resize-none rounded-2xl border border-slate-200 bg-transparent px-4 py-3 outline-none focus:border-[#A0845C] dark:border-white/10" />
        </label>
        <div className="mt-4 rounded-2xl border border-slate-200 p-4 dark:border-white/10">
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            <input value={form.featuredTitle} onChange={(event) => update("featuredTitle", event.target.value)} className="rounded-2xl border border-slate-200 bg-transparent px-4 py-3 outline-none focus:border-[#A0845C] dark:border-white/10" placeholder="Title" />
            <input value={form.featuredLink} onChange={(event) => update("featuredLink", event.target.value)} className="rounded-2xl border border-slate-200 bg-transparent px-4 py-3 outline-none focus:border-[#A0845C] dark:border-white/10" placeholder="External link" />
          </div>
          <textarea value={form.featuredDescription} onChange={(event) => update("featuredDescription", event.target.value)} className="mt-3 min-h-20 w-full resize-none rounded-2xl border border-slate-200 bg-transparent px-4 py-3 outline-none focus:border-[#A0845C] dark:border-white/10" placeholder="Short description" />
        </div>
        <div className="mt-4 rounded-2xl border border-slate-200 p-4 dark:border-white/10">
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-[1fr_auto]">
            <input type="email" value={email} onChange={(event) => setEmail(event.target.value)} className="rounded-2xl border border-slate-200 bg-transparent px-4 py-3 outline-none focus:border-[#A0845C] dark:border-white/10" placeholder="you@example.com" autoComplete="email" />
        ...
        <div className="mt-4 rounded-2xl border border-slate-200 p-4 dark:border-white/10">
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-[1fr_auto]">
            <input type="password" value={newPassword} onChange={(event) => setNewPassword(event.target.value)} className="rounded-2xl border border-slate-200 bg-transparent px-4 py-3 outline-none focus:border-[#A0845C] dark:border-white/10" placeholder="New password" />
        ...
        <div className="mt-4 rounded-2xl border border-slate-200 p-4 dark:border-white/10">
          ...
          {!user.verified ? (
            <>
              <textarea value={verificationReason} onChange={(event) => setVerificationReason(event.target.value)} className="mt-3 min-h-20 w-full resize-none rounded-2xl border border-slate-200 bg-transparent px-4 py-3 text-sm outline-none focus:border-[#A0845C] dark:border-white/10" placeholder="Why should this profile be verified?" />
      ...
      <section onPointerDown={(event) => event.stopPropagation()} className="modal-enter modal-scroll-pane w-full max-w-md rounded-t-[28px] border border-slate-200 bg-white p-5 pb-[max(20px,env(safe-area-inset-bottom))] shadow-2xl dark:border-white/10 dark:bg-[#0f1115] sm:rounded-[28px]">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            <span className="grid h-12 w-12 place-items-center rounded-2xl bg-[#A0845C]/10 text-[#A0845C]">
              <BadgeCheck size={24} />
            </span>
            <div>
        ...
        {user.verified ? <p className="mt-4 rounded-2xl bg-amber-50 p-3 text-sm font-semibold text-amber-800 dark:bg-amber-400/10 dark:text-amber-200">Verified account</p> : null}
        ...
        {isOwnProfile && !user.verified && derivedStatus !== "pending" ? (
          <div className="mt-5">
            ...
            <textarea value={requestReason} onChange={(event) => setRequestReason(event.target.value)} className="mt-2 min-h-28 w-full resize-none rounded-2xl border border-slate-200 bg-transparent px-4 py-3 text-sm outline-none focus:border-[#A0845C] dark:border-white/10" placeholder="Tell us what this account represents." />
...
export function ProfileView({ user, currentUserId, currentUserEmail, verificationStatus, verificationReason, users, posts, reactions, follows, blocks = [], mutes = [], onClose, onOpenProfile, onOpenPost, onLikePost, onRepostPost, onBookmarkPost, onFollowUser, onUpdateProfile, onUpdatePassword, onUpdateEmail, onRequestVerification, onShareProfile, onBlockUser, onUnblockUser, onReportUser, onCanvasFullscreenChange }: Props) {
  ...
  return (
    <div ref={scrollerRef} data-scroll-root="profile" onTouchStart={touchStart} onTouchEnd={touchEnd} className="modal-enter modal-scroll-pane fixed inset-0 z-20 overflow-y-auto bg-[#f4f2ee] pb-[max(96px,calc(env(safe-area-inset-bottom)+80px))] text-slate-950 dark:bg-[#080907] dark:text-white lg:left-72 lg:pb-0">
        ...
        <div className="min-w-0">
          <p className="flex items-center gap-1 font-bold">
            ...
            {(user.verified || isOwnProfile) ? (
              <button onClick={() => setVerificationOpen(true)} className="rounded-full text-slate-400 hover:text-[#A0845C] dark:text-slate-500 dark:hover:text-[#C4A265]" aria-label="Open verification details">
      ...
      <div className="mx-auto max-w-5xl pb-[max(96px,calc(env(safe-area-inset-bottom)+80px))]">
        ...
        <section className="px-4 py-6">
          {activeTab === "Canvas" ? (
            <div
              className={canvasFullscreen ? "fixed inset-0 z-[80] overflow-hidden bg-[#f4f2ee] dark:bg-[#080907]" : "relative h-[70vh] min-h-[480px] overflow-hidden rounded-3xl border border-slate-200 shadow-glass transition-all duration-300 ease-out dark:border-white/10"}
