...
export function AdminDashboard({ currentUser, allowCandidate = false, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[80] bg-[#f4f2ee] text-slate-950 dark:bg-[#080907] dark:text-white">
      <header className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-200 bg-white/90 px-4 py-3 backdrop-blur dark:border-white/10 dark:bg-slate-950/90">
        <div className="flex items-center gap-3">
          <span className="grid h-10 w-10 place-items-center rounded-2xl bg-[#A0845C]/10 text-[#A0845C]">
            <Shield size={20} />
          </span>
          <div>
            ...
            <section className="rounded-[28px] border border-slate-200 bg-white p-4 shadow-glass dark:border-white/10 dark:bg-[#111113]">
              <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <label className="relative block sm:w-72">
                  ...
                  <input value={search} onChange={(event) => setSearch(event.target.value)} className="h-11 w-full rounded-2xl border border-slate-200 bg-transparent pl-9 pr-3 text-sm outline-none focus:border-[#A0845C] dark:border-white/10" placeholder="Search users" />
                </label>
              </div>
                ...
                {filteredUsers.map((user) => (
                  <article key={user.id} className="flex items-center gap-3 rounded-2xl border border-slate-100 p-3 dark:border-white/10">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="flex items-center gap-1 truncate text-sm font-bold">{user.displayName}<VerifiedBadge verified={user.verified} size={14} />{user.isAdmin ? <Shield size={13} className="text-[#A0845C]" /> : null}</p>
                  ...
                  {verificationRequests.map((request) => (
                    <article key={request.id} className="rounded-2xl border border-slate-100 p-3 dark:border-white/10">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <BadgeCheck size={18} className={request.status === "approved" ? "text-[#A0845C]" : request.status === "rejected" ? "text-rose-500" : "text-amber-500"} />
                      </div>
                      {request.reason ? <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">{request.reason}</p> : null}
                      {request.status === "pending" ? (
                        <div className="mt-3 grid grid-cols-2 gap-2">
                          <button onClick={() => void reviewVerification(request, "approved")} className="flex items-center justify-center gap-1 rounded-xl bg-[#A0845C] px-3 py-2 text-xs font-bold text-white"><Check size={14} /> Approve</button>
