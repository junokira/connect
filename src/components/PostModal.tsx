          ...
          <div className="grid gap-6 p-5 lg:grid-cols-[1fr_340px]">
            <div>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {post.hashtags.map((tag) => <button key={tag} onClick={() => onHashtagClick?.(tag)} className="rounded-full bg-amber-50 px-3 py-1 text-sm font-medium text-amber-800 hover:underline dark:bg-amber-400/10 dark:text-amber-200">#{tag}</button>)}
              </div>
            ...
            <aside className="flex min-h-[320px] flex-col rounded-3xl border border-slate-200 bg-[#f4f2ee] dark:border-white/10 dark:bg-white/[0.04]">
              ...
              <form onSubmit={submit} className="sticky bottom-0 border-t border-slate-200 bg-white/95 p-3 pb-[max(12px,env(safe-area-inset-bottom))] backdrop-blur dark:border-white/10 dark:bg-slate-950/95">
                {replyingTo ? <p className="mb-2 text-xs font-bold text-[#A0845C]">Replying to @{replyingTo.authorUsername} <button type="button" onClick={() => setReplyingTo(undefined)} className="text-slate-400">cancel</button></p> : null}
