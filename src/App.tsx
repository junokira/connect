...
function AuthGate() {
  ...
  return (
    <main className="h-[100dvh] overflow-y-auto bg-[#f4f2ee] p-4 dark:bg-[#080907]">
      <form onSubmit={submit} className="mx-auto my-4 w-full max-w-md rounded-3xl border border-slate-200 bg-white p-6 shadow-glass dark:border-white/10 dark:bg-slate-950 sm:my-10">
        <p className="mb-1 text-2xl font-black text-slate-950 dark:text-white"><span className="text-[#A0845C]">VZN</span></p>
        <p className="mb-5 text-sm text-slate-500">Enter the VZN.</p>
          ...
          <>
            <div className="mb-4 grid gap-3 sm:grid-cols-2">
              <label className="block">
                ...
                <input value={displayName} onChange={(event) => setDisplayName(event.target.value)} className="w-full rounded-2xl border border-slate-200 bg-transparent px-4 py-3 outline-none focus:border-[#A0845C] dark:border-white/10" placeholder="Your name" />
              </label>
              <label className="block">
                <span className="mb-2 block text-sm font-semibold">Username</span>
                <input value={username} onChange={(event) => setUsername(event.target.value)} className="w-full rounded-2xl border border-slate-200 bg-transparent px-4 py-3 outline-none focus:border-[#A0845C] dark:border-white/10" placeholder="calvin" />
              </label>
            </div>
            <label className="mb-2 block text-sm font-semibold">Bio</label>
            <textarea value={bio} onChange={(event) => setBio(event.target.value)} className="mb-4 min-h-20 w-full resize-none rounded-2xl border border-slate-200 bg-transparent px-4 py-3 outline-none focus:border-[#A0845C] dark:border-white/10" placeholder="A little about you" />
          </>
        ) : null}
        <label className="mb-2 block text-sm font-semibold">Email</label>
        <input type="email" value={email} onChange={(event) => setEmail(event.target.value)} className="mb-4 w-full rounded-2xl border border-slate-200 bg-transparent px-4 py-3 outline-none focus:border-[#A0845C] dark:border-white/10" />
        <label className="mb-2 block text-sm font-semibold">Password</label>
        <input type="password" value={password} onChange={(event) => setPassword(event.target.value)} className="mb-4 w-full rounded-2xl border border-slate-200 bg-transparent px-4 py-3 outline-none focus:border-[#A0845C] dark:border-white/10" />
        ...
        {status ? <p className="mb-4 rounded-2xl bg-amber-50 p-3 text-sm text-amber-800 dark:bg-amber-400/10 dark:text-amber-200">{status}</p> : null}
        ...
        {mode === "signin" ? (
          <button type="button" disabled={!email || loading} onClick={() => void resetPassword()} className="mt-3 w-full text-center text-sm font-bold text-[#A0845C] disabled:opacity-50">
            Forgot password?
          </button>
        ) : null}
        ...
        <div className="grid gap-2">
          ...
          <div className="grid gap-2 sm:grid-cols-[1fr_auto]">
            <input value={phone} onChange={(event) => setPhone(event.target.value)} className="rounded-2xl border border-slate-200 bg-transparent px-4 py-3 text-sm outline-none focus:border-[#A0845C] dark:border-white/10" placeholder="+15555555555" />
...
function PublicSharedView({ post, author, profile, onSignIn }: { post?: Post; author?: User; profile?: User; onSignIn: () => void }) {
  return (
    <main className="min-h-[100dvh] bg-[#f4f2ee] p-4 text-slate-950 dark:bg-[#080907] dark:text-white">
      <div className="mx-auto max-w-xl py-8">
        <div className="mb-4 flex items-center justify-between">
          <div>
            <p className="text-xl font-black"><span className="text-[#A0845C]">VZN</span></p>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const description = activePost
      ...
      : activeProfile
        ...
        : "Explore AWAKEN CULT's visual world.";
  ...
  if (loading && !authed) {
    return (
      <main className="grid min-h-[100dvh] place-items-center bg-[#f4f2ee] p-4 text-slate-950 dark:bg-[#0a0b08] dark:text-white">
  ...
  return (
    <div
      className="flex h-[100dvh] overflow-hidden bg-[#f4f2ee] text-slate-950 dark:bg-[#080907] dark:text-white"
